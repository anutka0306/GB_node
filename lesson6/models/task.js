...
class Task {
    static async init() {
        ...
        await db.execute(`CREATE TABLE IF NOT EXISTS tasks (
            ...
            performer TINYINT UNSIGNED DEFAULT 0,
            PRIMARY KEY (id),
    ...
    static async getTasks(userId) {
        ...
        console.log(userId);
        const tasksForMe = await db.query('SELECT * FROM tasks where performer = ?', [userId]);

        const tasks = await db.query('SELECT * FROM tasks where owner = ?', [userId]);
        //console.log(tasks[0]);
        const result = [];
        result.push([tasks[0], tasksForMe[0]]);
        return result;
    }


    static async createTask(task) {
        const newTask = await db.query('INSERT INTO tasks(description, completed, owner, performer) VALUES (?, ?, ?, ?)', [task.description, task.completed=="true"?1:0, task.owner, task.performer]);
        return newTask;
    }
